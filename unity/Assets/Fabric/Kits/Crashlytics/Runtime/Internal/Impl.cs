...
namespace Fabric.Crashlytics.Internal
{
    ...
    internal class Impl
    {
        ...
        {
            MatchCollection matches = Regex.Matches(frameString, regex);
            if (matches.Count < 1)
            {
                return null;
            }
        ...
        protected static Dictionary<string, string>[] ParseStackTraceString(string stackTraceString)
        {
            ...
            foreach (string frameString in splitStackTrace)
            {
                Dictionary<string, string> parsedDict = ParseFrameString(regex, frameString);
                if (parsedDict != null)
                {
