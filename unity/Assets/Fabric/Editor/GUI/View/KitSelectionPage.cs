namespace Fabric.Internal.Editor.View
{
    ...
    internal class KitSelectionPage : Page
    {
        ...
        private readonly Func<List<ImportedKit>> listImportedKits;
        private readonly Func<KitsList> listAvailableKits;
        private readonly Action checkKitActivation;

        private readonly Action<KitsObject, ImportedKit, bool> onKitSelected;
        private readonly KeyValuePair<string, Action> back;
        private readonly KeyValuePair<string, Action> viewUpdateClickHandler;
        ...
        )
        {
            ...
            viewUpdateClickHandler = new KeyValuePair<string, Action>("View Update", showUpdatePage);
        ...
        public override void RenderImpl(Rect position)
        {
            ...
            foreach (KitsObject availableKit in availableKits)
            {
        ...
        private class KitRowData
        {
            ...
            {
                AvailableKit = availableKit;
                ImportedKit = importedKit;

                LatestVersion = availableKit.Version.ToString();
                IsUpToDate = KitUtils.IsUpToDate(availableKit, importedKit);

                Name = availableKit.Name;
                Status = KitStatus.Available;
                CurrentVersion = new System.Version().ToString(); // Default is 0.0

                if (importedKit != null)
                {
                    Name = importedKit.Name;
                    Status = KitUtils.StatusFor(importedKit);
                    CurrentVersion = importedKit.Instance.Version().ToString();
                }
            }
        }
    }
