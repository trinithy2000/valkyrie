    ...
    public class QuestDetailsScreen
    {
        public QuestDetailsScreen(QuestData.Quest q)
        {
            ...
            if (GameUtils.IsMoMGameType())
                screenUI.SetImage(CommonImageKeys.mom_bgnd_Investigator);
            else if (GameUtils.IsIAGameType())
                screenUI.SetImage(CommonImageKeys.ia_bgnd_Heroes);
            ...
            ui.SetText(q.name, GameUtils.ReturnValueGameType(Color.black, Color.white, Color.white));
            ui.SetFont(game.gameType.GetHeaderFont());
            ui.SetFontSize(GameUtils.ReturnValueGameType(UIScaler.GetMediumFont(), UIScaler.GetMediumFont(), UIScaler.GetBigFont(.8f)));
            ...
            ui.SetImage(q.image.Length > 0 ? ContentData.FileToTexture(Path.Combine(q.path, q.image)) : CommonImageKeys.default_img_quest);
            ...
            scrollUI.SetText(q.description, GameUtils.ReturnValueGameType(Color.black,Color.black, Color.white));
            ...
            scrollAutor.SetLocation(UIScaler.GetHCenter(-21.15f), UIScaler.GetBottom(-11), 14, 5.5f);
            scrollAutor.SetBGColor(Color.clear);
            //---------    
            ...
            dif_dur_ui.SetBGColor(Color.clear);

            if (GameUtils.IsIAGameType())
            {
                new UITitleBackGround(dif_dur_ui, CommonString.items);
            }
