...
public class PuzzleTower : Puzzle
{
    ...
    protected bool Equal(List<List<int>> state, List<List<int>> stateTwo)
    {
        ...
        for (int i = 0; i < state.Count; i++)
        {
            if (state[i].Count != stateTwo[i].Count)
            {
                return false;
            }
